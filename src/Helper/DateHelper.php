...
class DateHelper
{
	...
	{
		if ($hour < 24) {
			return Text::sprintf('COM_BOOKPRO_REFUND_HOUR_TXT', $hour);
		} else {
			$day = $hour / 24;
			$day = (int) $day;
			$new_hour = $hour % 24;
			return Text::sprintf('COM_BOOKPRO_REFUND_HOUR_DAY_TXT', $day, $new_hour);
		}
	}
	static function toLongDate($date)
	{
		if ($date == '0000-00-00 00:00:00')
			return 'N/A';
		$configJ = ComponentHelper::getParams('com_bookpro');
		$format = $configJ->get('date_long', 'D, d M Y');
		return Factory::getDate($date)->format($format);
	}
	...
	static function FactoryGetDateAndFormat($date, $format = "DATE_FORMAT_LC3")
	{
		if ($date == '0000-00-00 00:00:00') {
			return '';
		} else {
	...
	public static function formatSqlDate($date, $format = "DATE_FORMAT_LC3")
	{
		if ($date == '0000-00-00 00:00:00') {
			...
			return Text::_("COM_BOOKPRO_NOT_AVAILABLE");
		} else {
	...
	public static function formatMultiDate($date, $joomla_format = "DATE_FORMAT_LC3")
	{
		...
		for ($i = 0; $i < count($datearr); $i++) {
			$dateObj = Factory::getDate($datearr[$i]);
			$result[] = HTMLHelper::_('date', $dateObj, $format);
		}
		return $result;
	}
	...
	{
		return Factory::getDate(Factory::getDate($date)->format('Y-m-d 00:00:00'));
	}
	public static function dateEndDay($date)
	{
		return Factory::getDate(Factory::getDate($date)->format('Y-m-d 23:59:59'));
	}
	...
	// 			$jdate = Factory::getDate($date, $mainframe->getCfg('config'));
	// 			/* @var $date JDate */
	...
	static function createFromFormat($js_string_date)
	{
		...
		$date =  Date::createFromFormat($php_format, $js_string_date);
	...
	{
		$start_time = Factory::getDate()->format('Y-m-d') . ' ' . $start_time;
		$start_time = Factory::getDate($start_time);
		$end_time = Factory::getDate($end_time)->format('Y-m-d') . ' ' . $end_time;
		$end_time = Factory::getDate($end_time);
	...
	{
		$config = ComponentHelper::getParams('com_bookpro');
		//$value = Factory::getDate ()->format ( 'Y-m-d' ) . ' ' . $value;
		return Factory::getDate($value)->format(trim($config->get('timespace', 'h:i A')));
	}
}
