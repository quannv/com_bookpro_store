...
class BookProPaymentPlugin extends JPlugin 
{
	...
	function _getNotificationUrl($order_number=null){
		if($order_number)
			return JURI::root()."index.php?option=com_bookpro&controller=payment&task=postpayment&paction=process&method=".$this->_element."&order_number=".$order_number;
		else			
			return  JURI::root()."index.php?option=com_bookpro&controller=payment&task=postpayment&paction=process&method=".$this->_element;
	}
	function _getReturnUrl($order_number=null){
		if($order_number)
			return  JURI::root()."index.php?option=com_bookpro&controller=payment&task=postpayment&paction=display_message&method=".$this->_element.
    ...
    function _getOrder($order_number){
